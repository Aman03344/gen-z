...
const VerifyLoginOtp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(
        verifyLoginOtpThunk({ email, otp: otpString })
      ).unwrap();
  ...
  const handleResendOtp = async () => {
    ...
    try {
      await dispatch(resendLoginOtpThunk({ email })).unwrap();
      toast.dismiss(loadingToast);
      setTimer(60);
      setCanResend(false);
  ...
  const handleOtpChange = (index, value) => {
    ...
    if (value.length > 1) {
      ...
      const lastFilledIndex = Math.min(pastedValue.length, 5);
  ...
  useEffect(() => {
    if (isOtpVerificationSuccess && user && token) {
  ...
  }, [isOtpVerificationSuccess, user, token, navigate]);
